...
export default function Messages() {
  ...
  // Add a check for db availability
  const [dbReady, setDbReady] = useState(false)

  // Add this effect to check when db is ready
  useEffect(() => {
    if (db) {
      setDbReady(true)
    }
  ...
  // Modify the WebRTC hook initialization to only run when db is ready
  const [
  ...
  const handleStartVideoCall = async () => {
    ...
    if (!dbReady) {
      alert("Database connection is not ready. Please try again.")
      return
    }
