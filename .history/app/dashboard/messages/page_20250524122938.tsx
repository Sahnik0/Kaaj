...
export default function SimpleCallSystem() {
  const {
    user,
    getConversations,
    getMessages,
  ...
  // Simplified calling functionality
  const initiateCall = useCallback(
    (type: "audio" | "video") => {
      if (!selectedConversation || !window.ZegoUIKitPrebuilt) {
        console.error("Cannot initiate call: missing conversation or Zego SDK")
        return
      }
  ...
  const joinCall = useCallback(
    (type: "audio" | "video", roomId: string, recipientName?: string) => {
      ...
      console.log("Joining call:", { type, roomId, recipientName })
      ...
      try {
        ...
        console.log("Generated kit token, joining room...")
            ...
            {callStatus === "connecting" && (
              <div className="absolute inset-0 bg-gray-900/80 flex items-center justify-center">
                <div className="text-center">
                  ...
                  <p className="text-gray-300">Setting up call with {callRecipient?.name}</p>
                </div>
              </div>
            )}
          </div>
