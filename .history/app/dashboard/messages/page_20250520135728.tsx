...
import { Send, User, Search, X, Loader2, Video, PhoneCall } from "lucide-react"
...
export default function Messages() {
  ...
  useEffect(() => {
    ...
    // This would be your actual implementation to listen for call notifications
  ...
  // Mock function - replace with your actual Firebase implementation
  const listenForIncomingCalls = (userId: string, callback: (call: any) => void) => {
    // In a real implementation, this would set up a listener for incoming call notifications
    // For example, using Firebase onSnapshot to listen for new call documents
    return () => {}
  }
  ...
  const handleStartVideoCall = async () => {
    ...
    try {
      ...
      // In a real implementation, you would save this call to your database
      // For example, using Firebase to create a new call document
      await createCallNotification(call)

      // You might also want to send a push notification or use another real-time
      // mechanism to alert the user immediately
    } catch (error) {
  ...
  // Mock function - replace with your actual Firebase implementation
  const createCallNotification = async (call: any) => {
    // In a real implementation, this would create a call document in your database
    // For example, using Firebase to add a new document to a 'calls' collection
    console.log("Creating call notification:", call)
    return call
  }
  ...
  const updateCallStatus = async (callId: string, status: string) => {
    // In a real implementation, this would update the call document in your database
      ...
      <div className="fixed bottom-4 right-4 z-50 bg-white border-2 border-kaaj-500 rounded-lg shadow-lg p-4 w-80">
        ...
        <div className="flex gap-2">
          ...
          <Button
            ...
            onClick={() => {
              updateCallStatus(incomingCall.id, "accepted")
              setActiveCall(incomingCall)
              setIsInVideoCall(true)
              setIncomingCall(null)

              // If we're in a different conversation, switch to the call's conversation
              if (selectedConversation?.id !== incomingCall.conversationId) {
                ...
                <div className="absolute inset-0 z-10 bg-black flex flex-col">
                  <div className="flex justify-between items-center p-4 bg-black text-white">
                    <div className="flex items-center gap-2">
                      ...
                      <div>
                        <span>
                          {activeCall?.status === "accepted" ? "Video call with" : "Calling"}{" "}
                          {getOtherParticipantName(selectedConversation)}
                        </span>
                        {activeCall?.status !== "accepted" && (
                          <p className="text-xs text-gray-400">Waiting for answer...</p>
                        )}
                      </div>
                    </div>
                    <Button
                    ...
                    <div className="w-full h-full bg-gray-900 flex items-center justify-center">
                      {activeCall?.status === "accepted" ? (
                        ...
                        <div className="flex flex-col items-center">
                          ...
                          <p className="text-gray-400">Calling {getOtherParticipantName(selectedConversation)}...</p>
                        </div>
                      )}
                    </div>

                    {/* Self video (picture-in-picture) */}
                    <div className="absolute bottom-4 right-4 w-48 h-36 bg-gray-800 rounded-lg overflow-hidden border border-gray-700 flex items-center justify-center">
                  ...
                  <div className="p-4 bg-black flex justify-center gap-4">
                    ...
                    >
                      <svg
                        ...
                        className="text-white"
                      >
                        <path d="M23 7 L23 17 L17 12 Z"></path>
