...
import { useFirebaseReady } from "@/lib/firebase-ready-provider"
...
export default function Messages() {
  ...
  // Use the Firebase ready provider
  const { isReady, db } = useFirebaseReady()
  ...
  // Initialize WebRTC hook only when Firebase is ready
  const [
  ...
  ] = useWebRTC(user, isReady ? db : null, (status) => {
  ...
  useEffect(() => {
    ...
    return () => unsubscribe()
  }, [user, db, isReady, isInVideoCall, activeCall])

  // Add a check before starting a call
  const handleStartVideoCall = async () => {
    if (!selectedConversation) return
    if (!isReady) {
      toast({
        title: "Database Not Ready",
        description: "The database connection is not ready. Please try again in a moment.",
        variant: "destructive",
      })
      return
    }
  ...
  // Show database connection status if not ready
  if (!isReady) {
    return (
      <PageContainer>
        <div className="flex h-[calc(100vh-200px)] items-center justify-center flex-col">
          <Loader2 className="h-8 w-8 animate-spin text-kaaj-500 mb-4" />
          <p className="text-kaaj-600 font-medium">Establishing secure connection...</p>
          <p className="text-kaaj-400 text-sm mt-2">This may take a moment</p>
        </div>
      </PageContainer>
    )
  }
